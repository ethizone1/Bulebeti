...
const UpgradeRequiredScreen = ({ requiredTier, currentTier }) => {
  return (
    <div style={{
      ...
      <h2 style={{ fontSize: '22px', fontWeight: '800', margin: '0 0 10px', color: 'var(--primary)', letterSpacing: '0.5px' }}>
        {requiredTier.toUpperCase()} Plan Required
      </h2>
      <p style={{ color: 'var(--on-surface-variant)', fontSize: '14px', lineHeight: '1.6', margin: '0 0 28px', maxWidth: '380px' }}>
        This premium module is locked under the <strong>{requiredTier} Plan</strong>. Your restaurant is currently on the <strong>{currentTier} Plan</strong>.
      </p>
      ...
      <div style={{ display: 'flex', gap: '14px', justifyContent: 'center' }}>
        <button 
          onClick={() => {
            ...
            const contactBtn = buttons.find(b => b.textContent.includes('Contact Support'));
            if (contactBtn) {
              contactBtn.click();
            } else {
              alert('Please contact support@bulebet.com to upgrade your subscription.');
            }
          }}
          className="btn btn-primary"
          style={{ padding: '10px 20px', fontWeight: '700', borderRadius: '8px', cursor: 'pointer' }}
        >
          Upgrade Now
        </button>
        <button 
...
const AdminPage = ({ element: Element, minTier }) => {
  ...
  const currentTierImportance = tierImportance[tier] !== undefined ? tierImportance[tier] : 2; // Default to Platinum
  const requiredTierImportance = tierImportance[minTier] !== undefined ? tierImportance[minTier] : 0; // Default to Silver
    ...
    <LanguageProvider>
      <Router>
        ...
        <Routes>
          ...
          <Route path="/bulebet/:restaurantName">
            ...
            <Route 
              ...
              element={
                <AdminLayout>
                  <Routes>
                    <Route index element={<AdminPage element={AdminDashboard} minTier="Silver" />} />
                    <Route path="dashboard" element={<AdminPage element={AdminDashboard} minTier="Silver" />} />
                    <Route path="reservations" element={<AdminPage element={ReservationManagement} minTier="Gold" />} />
                    <Route path="catering" element={<AdminPage element={CateringManagement} minTier="Platinum" />} />
                    <Route path="menu" element={<AdminPage element={MenuManagement} minTier="Silver" />} />
                    <Route path="menu/add" element={<AdminPage element={AddMenuItem} minTier="Silver" />} />
                    <Route path="menu/edit/:itemId" element={<AdminPage element={EditMenuItem} minTier="Silver" />} />
                    <Route path="feedback" element={<AdminPage element={FeedbackManager} minTier="Premium" />} />
                    <Route path="testimonials" element={<AdminPage element={TestimonialsManager} minTier="Premium" />} />
                    <Route path="gallery" element={<AdminPage element={GalleryManager} minTier="Silver" />} />
                    <Route path="locations" element={<AdminPage element={LocationManagement} minTier="Platinum" />} />
                    <Route path="locations/add" element={<AdminPage element={AddLocation} minTier="Platinum" />} />
                    <Route path="events" element={<AdminPage element={EventsManager} minTier="Premium" />} />
                ...
                <Routes>
                  ...
                  <Route path="*" element={
                    <div style={{ textAlign: 'center', padding: '40px' }}>
                      ...
                      <p>This platform-level management module is currently under development.</p>
                    </div>
                  } />
                </Routes>
              </SuperAdminLayout>
