...
const CateringManagement = () => {
    ...
    return (
      ...
      >
        ...
        <p style={{ color: "var(--on-surface-variant)", marginBottom: "20px" }}>
          Catering Management requires the <strong>Platinum</strong> or{" "}
  ...
  const filtered = requests.filter((req) => {
    ...
    const matchDate =
      !dateFilter ||
      (req.date
        ? new Date(req.date).toISOString().startsWith(dateFilter)
        : false);
              ...
              <tbody className="border-top-0">
                ...
                {filtered.map((req) => (
                  <React.Fragment key={req._id}>
                    <tr
                      className={
                        expandedRowId === req._id ? "border-bottom-0" : ""
                      }
                      style={{ cursor: "pointer" }}
                      onClick={() =>
                        setExpandedRowId(
                          expandedRowId === req._id ? null : req._id,
                        )
                      }
                    >
                      <td className="text-muted font-monospace small">
                        <span
                          className="text-primary fw-bold me-2 d-inline-block"
                          style={{ width: "16px" }}
                        >
                      ...
                      <tr className="bg-light">
                        <td colSpan="7" className="p-4">
                          <div className="bg-white p-4 rounded-3 border shadow-sm">
                            <h4
                              className="text-warning text-uppercase small fw-bold mb-3"
                            ...
                            >
                              {req.details ||
                                "No additional details or menu items provided."}
                            </pre>
                          </div>
                        </td>
                      </tr>
