    ...
    return (
      ...
      >
        ...
        <p style={{ color: "var(--on-surface-variant)", marginBottom: "20px" }}>
          Multiple Hub Management requires the <strong>Platinum</strong> or{" "}
          ...
          {locations.map((loc) => {
            ...
            return (
              <div key={loc._id} className="col-12 col-md-6 col-xl-4">
                <div
                  className={`card h-100 border-0 shadow-sm rounded-4 ${isCurrent ? "border border-2 border-warning" : ""}`}
                >
                  <div className="card-body p-4 position-relative">
                    <span
                      className="badge bg-warning bg-opacity-10 text-warning position-absolute top-0 end-0 m-3 py-1 px-2 border border-warning border-opacity-25 rounded-3 fw-bold text-uppercase"
                    ...
                    <div className="border-top pt-3 d-flex justify-content-between align-items-end mt-auto">
                      <div>
                        ...
                        {loc.phone && (
                          <div className="text-muted small">📞 {loc.phone}</div>
                        )}
                      </div>
                      <div>
                        {isCurrent ? (
