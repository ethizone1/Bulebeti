...
const TeamManagement = () => {
  ...
  const fetchTeam = useCallback(async () => {
    try {
      ...
      const ownerIdStr = String(
        data.owner?._id || data.owner?.id || data.owner,
      );
      const isSuperAdmin =
        storedUser.role === "super-admin" || storedUser.role === "hub owner";
      ...
      if (ownerIdStr === storedUserId || isSuperAdmin) {
        ...
        const adminRecord = data.admins.find(
  ...
  if (loading)
    return <div className="p-5 text-center">Loading team data...</div>;

  const canManageTeam =
    isOwnerFlag || currentUserPermissions.includes("manage_team");
    ...
    <div className="team-management py-3">
      <div className="mb-4">
        ...
        <p className="text-muted">
          Manage who has access to your restaurant's dashboard and what they can
          do.
        </p>
      </div>
        ...
        <div className="alert alert-success border-0 shadow-sm rounded-4 p-4 mb-4">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <h6 className="fw-bold m-0 text-success">
              🎉 Sub-Admin Invitation Link Created!
            </h6>
            <button
              type="button"
              className="btn-close"
          ...
          <p className="small mb-3 text-dark">
            An invitation email/SMS dispatch was sent to{" "}
            <strong>{createdInvite.email}</strong> ({createdInvite.phone}). You
            can also copy and send this activation link directly to the
            sub-admin:
          </p>
            ...
            <div className="col-12">
              ...
              <div className="row g-2">
                {AVAILABLE_PERMISSIONS.filter(
                  (p) => TIER_LEVELS[restaurantTier] >= TIER_LEVELS[p.minTier],
                ).map((perm) => (
        ...
        <div className="table-responsive">
          <table className="table align-middle mb-0">
            <thead className="table-light">
              <tr>
                <th className="small text-uppercase text-muted fw-bold">
                  User
                </th>
                <th className="small text-uppercase text-muted fw-bold">
                  Permissions
                </th>
                <th className="small text-uppercase text-muted fw-bold text-end">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="border-top-0">
              {showOwner && (
                <tr>
                  ...
                  <td>
                    <span
                      className="badge bg-gold text-white"
                  ...
                  <td>
                    {editingAdminId === admin.user._id ? (
                      <div className="d-flex flex-wrap gap-2">
                        {AVAILABLE_PERMISSIONS.filter(
                          (p) =>
                            TIER_LEVELS[restaurantTier] >=
                            TIER_LEVELS[p.minTier],
                        ).map((perm) => (
                          <div
                            key={perm.id}
                          ...
                          >
                            ...
                            <label
                              ...
                              style={{
                                cursor: canManageTeam ? "pointer" : "default",
                                opacity: admin.permissions.includes(perm.id)
                                  ? 1
                                  : 0.6,
                              }}
                            >
                    ...
                    {canManageTeam && (
                      <div className="d-flex gap-2 justify-content-end">
                        {editingAdminId === admin.user._id ? (
                          <>
                            <button
                              onClick={() => handleSaveProfile(admin.user._id)}
