  ...
  React.useEffect(() => {
    const fetchFeedback = async () => {
      try {
        ...
        const fbRes = await fetch(`${config.API_URL}/api/feedback/restaurant/${restaurant._id}`);
    ...
    return (
      <div style={{ padding: '40px', textAlign: 'center', backgroundColor: 'var(--surface)', borderRadius: '12px', marginTop: '40px' }}>
        ...
        <p style={{ color: 'var(--on-surface-variant)', marginBottom: '20px' }}>
          Feedback Management requires the <strong>Platinum</strong> or <strong>Premium</strong> plan.
        </p>
        <Link to={`/bulebet/${restaurantName}/admin/settings`} style={{ padding: '10px 20px', backgroundColor: 'var(--primary)', color: 'white', textDecoration: 'none', borderRadius: '4px' }}>Upgrade Plan</Link>
      </div>
    );
  }
  ...
  return (
    <div className="feedback-manager py-3">
      ...
      <div className="card border-0 shadow-sm rounded-4 p-4">
        {feedbacks.map((fb) => (
          <div key={fb._id} className="d-flex flex-column flex-md-row gap-4 py-4 border-bottom last-border-0">
            ...
            <div className="d-flex flex-md-column gap-2" style={{ minWidth: '120px' }}>
              <span className={`badge w-100 ${fb.status === 'Published' ? 'bg-success bg-opacity-10 text-success' : (fb.status === 'Flagged' ? 'bg-danger bg-opacity-10 text-danger' : 'bg-secondary bg-opacity-10 text-secondary')}`}>
                {fb.status === 'Published' ? t('admin_fb_pub') : (fb.status === 'Flagged' ? t('admin_fb_flag') : t('admin_fb_rev'))}
              </span>
              <button className="btn btn-outline-secondary btn-sm flex-grow-1">{t('admin_fb_reply')}</button>
              <button 
                onClick={() => toggleStatus(fb._id, fb.status)}
                className="btn btn-outline-secondary btn-sm flex-grow-1"
              >
                {fb.status === 'Published' ? t('admin_fb_unpub') : t('admin_fb_dopub')}
              </button>
            </div>
          </div>
        ))}
