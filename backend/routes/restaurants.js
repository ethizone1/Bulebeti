...
router.get('/:slug', async (req, res) => {
  try {
    const restaurant = await Restaurant.findOne({ slug: { $regex: new RegExp(`^${req.params.slug}$`, 'i') } });
    if (!restaurant) {
...
router.post('/', auth, async (req, res) => {
  const { name, slug, description, address, phone, email, menuLayout, logoUrl, subscriptionTier } = req.body;

  try {
    // Check if restaurant with the same slug already exists
    const existing = await Restaurant.findOne({ slug: { $regex: new RegExp(`^${slug}$`, 'i') } });
    if (existing) {
      return res.status(400).json({ msg: 'A restaurant with this name or slug already exists.' });
    }
    ...
    if (userRestaurants.length > 0) {
      ...
      const tierRank = { 'Basic': 0, 'Silver': 1, 'Gold': 2, 'Platinum': 3, 'Premium': 4 };
      
      userRestaurants.forEach(r => {
        const t = r.subscriptionTier === 'Basic' ? 'Silver' : (r.subscriptionTier || 'Platinum');
    ...
    console.log(`[BACKEND] 🏢 New restaurant profile created: ${name} (slug: ${slug}) by User ID: ${req.user.id} with tier: ${subscriptionTier || 'Basic'}`);
    res.json(restaurant);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.put('/:slug/request-upgrade', auth, async (req, res) => {
  try {
    ...
    console.log(`[BACKEND] 🎫 Upgrade to ${tier} requested for restaurant: ${restaurant.name}`);
    res.json(restaurant);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.put('/admin/upgrade/:id', async (req, res) => {
  try {
    ...
    await restaurant.save();
    console.log(`[BACKEND] 👑 Admin updated restaurant ${restaurant.name} subscription tier to: ${restaurant.subscriptionTier}`);
    res.json(restaurant);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.put('/admin/edit/:id', async (req, res) => {
  try {
    ...
    const { name, slug, description, address, phone, email, menuLayout, subscriptionTier, status } = req.body;
...
router.put('/:slug', auth, async (req, res) => {
  try {
    ...
    // Update fields
    const { name, description, address, phone, email, menuLayout, logoUrl, subscriptionTier } = req.body;
    ...
    await restaurant.save();
    console.log(`[BACKEND] 🏢 Restaurant profile updated: ${restaurant.name} (slug: ${req.params.slug})`);
    res.json(restaurant);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
