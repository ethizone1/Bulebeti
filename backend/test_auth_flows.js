...
async function runTests() {
  ...
  await User.deleteMany({
    email: {
      $in: ["test-dual-login@example.com", "test-google-login@example.com"],
    },
  });
  ...
  const server = app.listen(PORT, async () => {
    ...
    try {
      ...
      await testUser.save();
      console.log(
        "💾 Created test user with email: test-dual-login@example.com, phone: 1234567890",
      );
      ...
      const phoneLoginRes = await fetch(
        `http://localhost:${PORT}/api/auth/login`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const googleSignupRes = await fetch(
        `http://localhost:${PORT}/api/auth/google`,
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            token: "mock-google-token-test-google-login@example.com",
          }),
        },
      );
      ...
      if (
        googleSignupRes.ok &&
        googleSignupData.token &&
        googleSignupData.user.email === "test-google-login@example.com"
      ) {
        console.log(
          "✅ PASS: Google Auto-Signup successful. User created & logged in.",
        );
      } else {
      ...
      const createdGoogleUser = await User.findOne({
        email: "test-google-login@example.com",
      });
      if (
        createdGoogleUser &&
        createdGoogleUser.googleId.startsWith("mock-google-id-")
      ) {
        console.log("✅ PASS: Google user verified in MongoDB.");
      } else {
        console.error(
          "❌ FAIL: Google user not found in MongoDB or incorrect googleId.",
        );
      }
      ...
      if (googleLoginRes.ok && googleLoginData.token) {
        ...
        console.error(
          "❌ FAIL: Google Login for existing user failed:",
          googleLoginData,
        );
      }
      ...
      const badPasswordRes = await fetch(
        `http://localhost:${PORT}/api/auth/login`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (
        !badPasswordRes.ok &&
        badPasswordData.msg &&
        badPasswordData.msg.includes("Google Login")
      ) {
        console.log(
          "✅ PASS: Blocked standard login for Google-only user. Msg:",
          badPasswordData.msg,
        );
      } else {
        console.error(
          "❌ FAIL: Standard login was not blocked or returned incorrect msg:",
          badPasswordData,
        );
      }
    } catch (err) {
