async function testUpgradeFlow() {
  ...
  try {
    ...
    console.log(
      "✅ Admin user registered & verified successfully. Token received.",
    );
    ...
    if (!createRestRes.ok) {
      throw new Error(
        `Restaurant creation failed: ${await createRestRes.text()}`,
      );
    }

    const restaurant = await createRestRes.json();
    console.log(
      `✅ Restaurant created. Initial active tier: ${restaurant.subscriptionTier}, Slug: ${restaurant.slug}`,
    );

    // 3. Request Upgrade to Gold
    console.log("\n[Step 3] Submitting upgrade request for Gold tier...");
    const upgradeReqRes = await fetch(
      `http://localhost:5000/api/restaurants/${slug}/request-upgrade`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    if (restaurant.email !== "restaurant_contact@test.com") {
      throw new Error(
        `Expected email to be restaurant_contact@test.com, but got: ${restaurant.email}`,
      );
    }
    ...
    console.log(
      `✅ Upgrade requested. pendingTierRequest: ${afterRequest.pendingTierRequest}`,
    );

    if (afterRequest.pendingTierRequest !== "Premium") {
      throw new Error(
        `Expected pendingTierRequest to be Premium, but got: ${afterRequest.pendingTierRequest}`,
      );
    }
    ...
    const superAdminToken = jwt.sign(
      { user: { id: "superadmin_test_id", role: "super-admin" } },
      process.env.JWT_SECRET,
      { expiresIn: "1h" },
    );
    const approveRes = await fetch(
      `http://localhost:5000/api/restaurants/admin/upgrade/${restaurant._id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    console.log(
      `✅ Upgrade approved. subscriptionTier: ${afterApprove.subscriptionTier}, pendingTierRequest: '${afterApprove.pendingTierRequest}'`,
    );

    if (
      afterApprove.subscriptionTier !== "Premium" ||
      afterApprove.pendingTierRequest !== ""
    ) {
      throw new Error(
        `Unexpected tier state after approval: tier=${afterApprove.subscriptionTier}, pending=${afterApprove.pendingTierRequest}`,
      );
    }
