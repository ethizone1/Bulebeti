    ...
    try {
      ...
      if (!restaurant) {
        restaurant = new Restaurant({
          name: "Event Test Rest",
          slug: "event-test-rest",
          ownerId: admin._id,
        });
      ...
      console.log(
        `✅ GET /api/events returned HTTP ${getRes.status}. Events count: ${getEvents.length}`,
      );
      ...
      if (createRes.ok && newEvent._id) {
        console.log(
          `✅ Event created successfully! ID: ${newEvent._id}, Title: ${newEvent.title}`,
        );
      } else {
      ...
      const updateRes = await fetch(
        `http://localhost:${PORT}/api/events/${newEvent._id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (updateRes.ok && updatedEvent.title.includes("Updated")) {
        console.log(
          `✅ Event updated successfully! New Title: ${updatedEvent.title}`,
        );
      } else {
        throw new Error(
          `Failed to update event: ${JSON.stringify(updatedEvent)}`,
        );
      }
      ...
      if (deleteRes.ok && deleteData.msg === "Event removed") {
        ...
        throw new Error(
          `Failed to delete event: ${JSON.stringify(deleteData)}`,
        );
      }
